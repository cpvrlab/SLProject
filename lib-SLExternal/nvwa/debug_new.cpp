...
#if _DEBUG_NEW_TAILCHECK
    memset((char*)usRT_ptr + size, _DEBUG_NEW_TAILCHECK_cHAR,
 ...
 */
static void free_pointer(void* usRT_ptr, void* addr, bool is_array)
{
    if (usRT_ptr == NULL)
        return;
    new_ptr_list_t* ptr =
 ...
 */
void debug_new_recorder::_M_process(void* usRT_ptr)
{
    if (usRT_ptr == NULL)
        return;
