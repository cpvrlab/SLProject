                                      ...
                                      bool&  return_value)
{
    ...
    string scrComplete = srcVersion + source;
                             ...
                             GLuint fragShaderID)
{
    ...
    GLuint programHandle = glCreateProgram();
    glAttachShader(programHandle, vertShaderID);
    glAttachShader(programHandle, geomShaderID);
    glAttachShader(programHandle, fragShaderID);
    glLinkProgram(programHandle);
                               ...
                               const std::array<GLfloat, 4>& borderColor)
{
    ...
    if ((GLint)std::min(std::min((unsigned)files.size(), first.height()), first.width()) > maxSize)
    {
...
{
    string versionStr = string((const char*)glGetString(GL_SHADING_LANGUAGE_VERSION));
