...
MyVector createABigArray() { MyVector am(1000); return am; }
...
template<typename T> T addCpp11(T v) {return v;}
template<typename T, typename... Args> T addCpp11(T first, Args... args) {return first + addCpp11(args...);}

// C++17: A lot simpler, so forget the C++11 version
template<typename ...Args> auto addCpp17(Args ...args) {return (args + ...);}
...
struct absIntCompair {bool operator()(const int a, int b) { return abs(a) < abs(b); }};

///////////////////////////////////////////////////////////////////////////////
void new_lambda_expressions()
{
    cout << "\nLamdas: Simple quick function:-------------------------------------\n";
    auto HelloWorld = []() { cout << "Hello world"; };
    ...
    sort(vi.begin(), vi.end(), [](int a, int b) {return abs(a) < abs(b); });
    for (auto i : vi) cout << i << ",";

    auto fsum1 = [](int a, int b) {return a + b; };
    cout << "\nLambda function object with 2 int: fsum1(4,2) = " << fsum1(4, 2);

    int a = 4;
    auto func1 = [&](int b) {a++; return a + b; }; // a is accessed by reference
    cout << "\nLambda function w. captured variable by reference:     func1(2) = " << func1(2) << ", a=" << a;

    auto func2 = [=](int b) {return a + b; }; // a is accessed by value
    cout << "\nLambda function w. captured variable by value:         func2(2) = " << func2(2) << ", a=" << a;

    auto func3 = [=](int b) mutable {a++; return a + b; }; // a is accessed by mutable value
    cout << "\nLambda function w. captured variable by mutable value: func3(2) = " << func3(2) << ", a=" << a;

    auto func4 = [] {printf("\nLambda functions can call any other function."); };
    func4();

    function<int(int)> func5 = [&func5](int n) {return n <= 1 ? 1 : n * func5(n - 1); };
...
void new_type_deduction()
{
    ...
    cout << "\nauto:--------------------------------------------------------------\n";
    auto  i1 = 1;                cout << "This is an int: " << i1 << endl;
    auto  f1 = 1.1f;             cout << "This is a float: " << f1 << endl;
    auto  d1 = 1.1;              cout << "This is a double: " << d1 << endl;
    auto* b1 = new B();          cout << "This is an instance of B:" << b1->getA() << endl;
    auto  e1 = mapemail.begin(); cout << "This is an email address: " << (*e1).second << endl;

    cout << "\nC++14 auto as return type:-----------------------------------------\n";
    auto a = funcReturnsAuto(); cout << "funcReturnsAuto return: " << typeid(a).name() << endl;

    // New: decltype for type deduction from a variable
    cout << "\ndecltype:----------------------------------------------------------\n";
    decltype(i1) i2 = 1;          cout << "This is an int: " << i2 << endl;
    decltype(f1*i1) f2 = i1*f1;   cout << "This is an float: " << f2 << endl;
    ...
    for (auto i : arrayi) cout << i << ","; cout << endl;
    for (auto i : mapemail) cout << i.first << ": " << i.second << ","; cout << endl;
    for (auto i : vectori) cout << i << ","; cout << endl;
    for (auto i : {1,2,3,5,8,13,21,34}) cout << i << ","; cout << endl;
}
///////////////////////////////////////////////////////////////////////////////
...
void new_basic_types_and_type_traits()
{
    ...
    cout << "\nNew int types:-----------------------------------------------------\n";
    int8_t   i8 = -1;    cout << "This is an  8 bit integer:          " << i8   << endl;
    uint8_t  ui8 = 255;  cout << "This is an  8 bit unsigned integer: " << ui8  << endl;
    int16_t  i16 =-1000; cout << "This is an 16 bit integer:          " << i16  << endl;
    uint16_t iu16= 1000; cout << "This is an 16 bit unsigned integer: " << iu16 << endl;
    int32_t  i32 = 1000; cout << "This is an 32 bit integer:          " << i32  << endl;
    uint32_t ui32= 1000; cout << "This is an 32 bit unsigned integer: " << ui32 << endl;
    int64_t  i64 = 1000; cout << "This is an 64 bit integer:          " << i64  << endl;
    uint64_t ui64= 1000; cout << "This is an 64 bit unsigned integer: " << ui64 << endl;
...
CounterNTS cntNTS;
void incNTS() { for (int i = 0; i < 10000; ++i) cntNTS.inc(); }
void decNTS() { for (int i = 0; i < 10000; ++i) cntNTS.dec(); }

CounterTS1 cntTS1;
void incTS1() { for (int i = 0; i < 10000; ++i) cntTS1.inc(); }
void decTS1() { for (int i = 0; i < 10000; ++i) cntTS1.dec(); }

CounterTS2 cntTS2;
void incTS2() { for (int i = 0; i < 10000; ++i) cntTS2.inc(); }
void decTS2() { for (int i = 0; i < 10000; ++i) cntTS2.dec(); }
...
void new_random_generators()
{
    ...
    for (int i=0; i<5; ++i) cout << rnd() << ","; cout << endl;
}
///////////////////////////////////////////////////////////////////////////////
...
{
public:
    class Meter {}; // a tag class for avoiding implicit constructor call with double
...
void new_userdefined_literals()
{
    ...
    Length len2 = 2.0_in + 2.0_km;  cout << len2.lenghtInMeters() << " m" << endl;
