 ...
 */
static bool
print_position_from_addr(const void* addr)
{
...
#        endif
        char* cmd = (char*)alloca(strlen(new_progname) + exeext_len + sizeof addr2line_cmd - 1 + sizeof ignore_err - 1 + sizeof(void*) * 2 + 4 /* SP + "0x" + null */);
 ...
 */
static bool
print_position_from_addr(const void*)
{
    return false;
}
 ...
 */
static void
print_position(const void* ptr, int line)
{
 ...
 */
static void*
alloc_mem(size_t size, const char* file, int line, bool is_array)
{
    assert(line >= 0);
...
#    endif
    ...
    void* usRT_ptr = (char*)ptr + ALIGNED_LIST_ITEM_SIZE;
#    if _DEBUG_NEW_FILENAME_LEN == 0
    ptr->file = file;
#    else
    if (line)
...
#    if _DEBUG_NEW_TAILCHECK
    memset((char*)usRT_ptr + size, _DEBUG_NEW_TAILCHECK_cHAR, _DEBUG_NEW_TAILCHECK);
#    endif
    if (new_verbose_flag)
    {
        fast_mutex_autolock lock(new_output_lock);
 ...
 *
 * @param usRT_ptr   pointer to the previously allocated memory
 * @param addr      pointer to the caller
 * @param is_array  flag indicating whether it is invoked by a
 *                  <code>delete[]</code> call
 */
static void
free_pointer(void* usRT_ptr, void* addr, bool is_array)
{
    if (usRT_ptr == NULL)
        return;
    new_ptr_list_t* ptr =
      (new_ptr_list_t*)((char*)usRT_ptr - ALIGNED_LIST_ITEM_SIZE);
    if (ptr->magic != DEBUG_NEW_MAGIC)
    {
        {
            fast_mutex_autolock lock(new_output_lock);
            fprintf(new_output_fp, "delete%s: invalid pointer %p (", is_array ? "[]" : "", usRT_ptr);
...
check_leaks()
{
    ...
    {
        const char* const usRT_ptr = (char*)ptr + ALIGNED_LIST_ITEM_SIZE;
        if (ptr->magic != DEBUG_NEW_MAGIC)
        {
            fprintf(new_output_fp,
 ...
 *
 * @param usRT_ptr  pointer returned by a new-expression
 */
void
debug_new_recorder::_M_process(void* usRT_ptr)
{
    if (usRT_ptr == NULL)
        return;
    ...
    size_t offset = (char*)usRT_ptr - (char*)NULL;
    if (offset % PLATFORM_MEM_ALIGNMENT != 0)
    {
        ...
        usRT_ptr = (char*)usRT_ptr - sizeof(size_t);
    }
 ...
 */
void
operator delete(void* ptr, const char* file, int line)_NOEXCEPT
{
    if (new_verbose_flag)
    {
        fast_mutex_autolock lock(new_output_lock);
