...
ftplib::ftplib()
{
    ...
    if (mp_ftphandle == nullptr) perror("calloc");
    mp_ftphandle->buf = static_cast<char*>(malloc(FTPLIB_BUFSIZ));
    if (mp_ftphandle->buf == nullptr)
    {
        perror("calloc");
        free(mp_ftphandle);
    }
...
int ftplib::readline(char* buf, int max, ftphandle* ctl)
{
    ...
    do
    {
        ...
        if ((ctl->dir == FTPLIB_CONTROL) && (mp_ftphandle->logcb != nullptr))
        {
            *((ctl->cput) + x) = '\0';
...
int ftplib::Connect(const char* host)
{
    ...
    if (pnum == nullptr)
    {
        if ((pse = getservbyname("ftp", "tcp")) == nullptr)
        {
...
int ftplib::FtpSendCmd(const char* cmd, char expresp, ftphandle* nControl)
{
    ...
    if (mp_ftphandle->logcb != nullptr) mp_ftphandle->logcb(buf, mp_ftphandle->cbarg, false);
...
int ftplib::FtpClose(ftphandle* nData)
{
    ...
    if (nData->dir == FTPLIB_WRITE)
    {
        if (nData->buf != nullptr) writeline(nullptr, 0, nData);
    }
...
int ftplib::NegotiateEncryption()
{
    ...
    if (mp_ftphandle->certcb != nullptr)
    {
