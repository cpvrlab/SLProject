...
class SLMat4
{
  ...
  const T&          operator    ()(int row, int col)const {return _m[4*col+row];}
        ...
        void        perspectiveUncenteredPP(const T w, const T h, const T fx, const T fy, 
                                 const T cx, const T cy, const T n, const T f);
...
*/
template<class T>
void SLMat4<T>::perspectiveUncenteredPP(const T w, const T h, const T fx, const T fy, 
    const T cx, const T cy, const T n, const T f)
{
    //_m[0]=2*fx/w;     _m[4]=0;          _m[8] = 0;           _m[12]=0;
    //_m[1]=0;          _m[5]=2*fy/h;     _m[9] = 0;           _m[13]=0;
    //_m[2]=2*(cx/w)-1; _m[6]=2*(cy/h)-1; _m[10]=-(f+n)/(f-n); _m[14]=-1;
    //_m[3]=0;          _m[7]=0;          _m[11]=-2*f*n/(f-n); _m[15]=0;

    //_m[0]=2*fx/w;     _m[4]=0;          _m[8] = 2*(cx/w)-1;           _m[12]=0;
    //_m[1]=0;          _m[5]=2*fy/h;     _m[9] = 2*(cy/h)-1;           _m[13]=0;
    //_m[2]=0;          _m[6]=0;          _m[10]=-(f+n)/(f-n);          _m[14]=(-2*f*n)/(f-n);
    //_m[3]=0;          _m[7]=0;          _m[11]=-1;                    _m[15]=0;
