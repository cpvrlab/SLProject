...
*/
SLbool SLInputManager::pollAndProcessEvents(const std::vector<SLSceneView*>& svs)
{
    // process system events first
    SLbool consumedEvents = processQueuedEvents(svs);
...
SLbool SLInputManager::processQueuedEvents(const std::vector<SLSceneView*>& svs)
{
    ...
    while (!q.empty())
    {
        ...
        SLSceneView* sv = svs[(SLuint)e->svIndex];
