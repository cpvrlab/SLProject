...
bool LoopClosing::DetectLoop()
{
    ...
    const vector<SLCVKeyFrame*> vpConnectedKeyFrames = mpCurrentKF->GetVectorCovisibleKeyFrames();
    ...
    {
        SLCVKeyFrame* pCandidateKF = vpCandidateKFs[i];

        set<SLCVKeyFrame*> spCandidateGroup = pCandidateKF->GetConnectedKeyFrames();
        ...
        {
            set<SLCVKeyFrame*> sPreviousGroup = mvConsistentGroups[iG].first;

            bool bConsistent = false;
            for(set<SLCVKeyFrame*>::iterator sit=spCandidateGroup.begin(), send=spCandidateGroup.end(); sit!=send;sit++)
            {
...
bool LoopClosing::ComputeSim3()
{
    ...
    {
        SLCVKeyFrame* pKF = mvpEnoughConsistentCandidates[i];
    ...
    while(nCandidates>0 && !bMatch)
    {
        for(int i=0; i<nInitialCandidates; i++)
        {
            ...
            if(!Scm.empty())
            {
                vector<SLCVMapPoint*> vpMapPointMatches(vvpMapPointMatches[i].size(), static_cast<SLCVMapPoint*>(NULL));
    ...
    vector<SLCVKeyFrame*> vpLoopConnectedKFs = mpMatchedKF->GetVectorCovisibleKeyFrames();
    vpLoopConnectedKFs.push_back(mpMatchedKF);
    mvpLoopMapPoints.clear();
    for(vector<SLCVKeyFrame*>::iterator vit=vpLoopConnectedKFs.begin(); vit!=vpLoopConnectedKFs.end(); vit++)
    {
        SLCVKeyFrame* pKF = *vit;
        vector<SLCVMapPoint*> vpMapPoints = pKF->GetMapPointMatches();
...
void LoopClosing::SearchAndFuse(const KeyFrameAndPose &CorrectedPosesMap)
{
    ...
    for(KeyFrameAndPose::const_iterator mit=CorrectedPosesMap.begin(), mend=CorrectedPosesMap.end(); mit!=mend;mit++)
    {
        ...
        vector<SLCVMapPoint*> vpReplacePoints(mvpLoopMapPoints.size(),static_cast<SLCVMapPoint*>(NULL));
        ...
        for(int i=0; i<nLP;i++)
        {
            SLCVMapPoint* pRep = vpReplacePoints[i];
            if(pRep)
            {
...
void LoopClosing::RunGlobalBundleAdjustment(unsigned long nLoopKF)
{
    ...
    // We need to propagate the correction through the spanning tree
    {
        ...
        if(!mbStopGBA)
        {
            ...
            {
                SLCVKeyFrame* pKF = lpKFtoCheck.front();
                const set<SLCVKeyFrame*> sChilds = pKF->GetChilds();
                cv::Mat Twc = pKF->GetPoseInverse();
                for(set<SLCVKeyFrame*>::const_iterator sit=sChilds.begin();sit!=sChilds.end();sit++)
                {
                    SLCVKeyFrame* pChild = *sit;
            ...
            // Correct MapPoints
            const vector<SLCVMapPoint*> vpMPs = mpMap->GetAllMapPoints();
