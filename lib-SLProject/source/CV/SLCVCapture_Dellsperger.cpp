...
inline uint8_t clipFToUInt8(float val)
{
    float result = val;
...
void *copyToBuffer(void *arg)
{
    ...
    for (int row = 0; row < rowCount; ++row)
    {
        ...
        uint8_t* grayPix = info->grayRow;
        uint8_t* yPix = info->yRow;
        uint8_t* uPix = info->uRow;
        uint8_t* vPix = info->vRow;
                                ...
                                SLuchar* v, int vSize, int vPixStride, int vLineStride)
{
    ...
    */
    ...
    uint8_t *destRow = lastFrame.data;
    uint8_t *grayRow = lastFrameGray.data;

    int destPixSize = 3;
    int32_t stride = dstW * destPixSize;

    int grayPixSize = 1;
    int grayStride = dstW * grayPixSize;

    int destRowStride = stride;
    int grayRowStride = dstW;
    if (mirrorH) {
        destRow += (dstH - 1) * stride;
    ...
    uint8_t *yRow = y +     cropH*yLineStride +     cropW*yPixStride;
    uint8_t *uRow = u + halfCropH*uLineStride + halfCropW*uPixStride;
    uint8_t *vRow = v + halfCropH*vLineStride + halfCropW*vPixStride;
