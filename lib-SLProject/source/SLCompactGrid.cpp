...
SLVec3i SLCompactGrid::containingVoxel(const SLVec3f &p) const
{
    ...
    if (pos.x >= (SLint)_size.x) pos.x = (SLint)_size.x - 1; if (pos.x < 0) pos.x = 0;
    if (pos.y >= (SLint)_size.y) pos.y = (SLint)_size.y - 1; if (pos.y < 0) pos.y = 0;
    if (pos.z >= (SLint)_size.z) pos.z = (SLint)_size.z - 1; if (pos.z < 0) pos.z = 0;
...
void SLCompactGrid::ifTriangleInVoxelDo(triVoxCallback callback)
{
    ...
    {
        auto index  = [&](SLuint j) { return _m->I16.size() ? _m->I16[i*3+j] : _m->I32[i*3+j]; };
...
void SLCompactGrid::updateStats (SLNodeStats &stats)
{
    ...
    stats.numBytesAccel += _m->I16.size() ? SL_sizeOfVector(_triangleIndexes16):
		...
		{  //Calculate the intersection point with the AABB
			...
			while (!wasHit)
			{
				...
				if (tMaxX < tMaxY)
				{
					...
					{
						startVox.x += stepX;
                        if (startVox.x >= (SLint)_size.x || startVox.x < 0) return wasHit; // dropped outside grid
						tMaxX += tDeltaX;
                        voxID += (SLuint)incIDX;
						tMax = tMaxX;
					}
					...
					{
						startVox.z += stepZ;
                        if (startVox.z >= (SLint)_size.z || startVox.z < 0) return wasHit; // dropped outside grid
						tMaxZ += tDeltaZ;
                        voxID += (SLuint)incIDZ;
						tMax = tMaxZ;
					}
				...
				else
				{
					...
					{
						startVox.y += stepY;
                        if (startVox.y >= (SLint)_size.y || startVox.y < 0) return wasHit; // dropped outside grid
						tMaxY += tDeltaY;
                        voxID += (SLuint)incIDY;
						tMax = tMaxY;
					}
