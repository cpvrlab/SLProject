...
SLColorLUT::SLColorLUT(SLAssetManager* assetMgr,
...
void SLColorLUT::colors(SLColorLUTType lutType)
{
    assert(lutType != CLUT_custom && "SLColorLUT::colors: Custom LUT now allowed");
    ...
    switch (lutType)
    {
        ...
        case CLUT_DAYLIGHT:
            ...
            break;
        default:
            SL_EXIT_MSG("SLColorLUT::colors: undefined color LUT.");
            break;
    }
}
//-----------------------------------------------------------------------------
//! Generates the full 256 value LUT as 1x256 RGBA texture
void SLColorLUT::generateTexture()
{
    assert(_length > 1);
    assert(!_colors.empty() &&
           "SLColorLUT::generateTexture: Not enough color values.");
    ...
    if (!_alphas.empty())
    {
        ...
        if (_alphas.front().pos < 0.0f)
            SL_EXIT_MSG("SLColorLUT::generateTexture: Lower alpha pos below 0");
        if (_alphas.back().pos > 1.0f)
            SL_EXIT_MSG("SLColorLUT::generateTexture: Upper alpha pos above 1");
        if (_colors.front().pos < 0.0f)
            SL_EXIT_MSG("SLColorLUT::generateTexture: Lower color pos below 0");
        if (_colors.back().pos > 1.0f)
            SL_EXIT_MSG("SLColorLUT::generateTexture: Upper color pos above 1");
        ...
        for (SLuint a = 0; a < _alphas.size() - 1; ++a)
            if ((_alphas[a + 1].pos - _alphas[a].pos) < delta)
                SL_EXIT_MSG("SLColorLUT::generateTexture: Alpha position deltas to small.");
    ...
    for (SLuint c = 0; c < _colors.size() - 1; ++c)
        if ((_colors[c + 1].pos - _colors[c].pos) < delta)
            SL_EXIT_MSG("SLColorLUT::generateTexture: Color position deltas to small.");
