...
void SLSceneView::build2DInfoGL()
{
    ...
    if (_showStatsTiming)
    {
        ...
        SLfloat featureTrTimePC = SL_clamp(s->trackingTimesMS().average() / s->frameTimesMS().average()*100.0f, 0.0f,100.0f);
        ...
        sprintf(m+strlen(m), ">> Feature Description Extraction Time: %4.1f ms (%0.0f%%)\\n", s->featureTimesMS().average(), featureTimePC);
        sprintf(m+strlen(m), ">> Feature Tracking Time: %4.1f ms (%0.0f%%)\\n", s->trackingTimesMS().average(), featureTrTimePC);
