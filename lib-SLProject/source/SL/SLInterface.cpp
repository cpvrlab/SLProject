...
yaw and roll.
*/
void slRotationVec(float v1, float v2, float v3)
{
    SLScene::current->onRotationVec(SLVec3f(v1, v2, v3));
}
//-----------------------------------------------------------------------------
/*! Global event handler for device rotation change with column major 3x3 matrix
*/
void slRotationMat(float m0, float m3, float m6,
                   float m1, float m4, float m7,
                   float m2, float m5, float m8)
{
    SLScene::current->onRotationMat(SLMat3f(m0, m3, m6,
                                            m1, m4, m7,
                                            m2, m5, m8));
}
//-----------------------------------------------------------------------------
...
*/
void slRotationQuat(float qX, float qY, float qZ, float qW)
{
    SLScene::current->onRotationQuat(SLQuat4f(qX, qY, qZ, qW));
}
//-----------------------------------------------------------------------------
