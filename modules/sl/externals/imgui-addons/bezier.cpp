...
{
  enum { STEPS = 256 };
  ImVec2 Q[4] = {{StaEnd[0], StaEnd[1]}, {P[0], P[1]}, {P[2], P[3]}, {StaEnd[2], StaEnd[3]}};
...
int Bezier(const char* label, float P[5], float StaEnd[4]) {
  ...
  // header and spacing
  int changed = 0;
  changed     = ImGui::SliderFloat4("Controls points P1 & P2", P, 0, 1, "%.3f", ImGuiSliderFlags_None);

  // header and spacing
  int changedStaEnd = 0;
  changedStaEnd     = SliderFloat4("Start and end points", StaEnd, 0, 1, "%.3f", ImGuiSliderFlags_None);
  ...
  // eval curve
  ImVec2 Q[4] = {{StaEnd[0], StaEnd[1]}, {P[0], P[1]}, {P[2], P[3]}, {StaEnd[2], StaEnd[3]}}; // Need to change end point
  ...
  ImVec2 pStart = ImVec2(StaEnd[0], 1 - StaEnd[1]) * (bb.Max - bb.Min) + bb.Min;
  ImVec2 p1 = ImVec2(P[0], 1 - P[1]) * (bb.Max - bb.Min) + bb.Min;
  ImVec2 p2 = ImVec2(P[2], 1 - P[3]) * (bb.Max - bb.Min) + bb.Min;
  ImVec2 pEnd   = ImVec2(StaEnd[2], 1 - StaEnd[3]) * (bb.Max - bb.Min) + bb.Min;
  DrawList->AddLine(pStart, p1, ImColor(white), LINE_WIDTH);
  DrawList->AddLine(pEnd, p2, ImColor(white), LINE_WIDTH);
