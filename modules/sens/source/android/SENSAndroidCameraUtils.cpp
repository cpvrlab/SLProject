...
{
    ASSERT(lens.tag == ACAMERA_LENS_FACING, "Wrong tag(%#x) of %s to %s", lens.tag, GetTagStr((acamera_metadata_tag_t)lens.tag), __FUNCTION__);
    LOGI("LensFacing: tag(%#x), type(%d), count(%d), val(%#x)", lens.tag, lens.type, lens.count, lens.data.u8[0]);
}
...
#define MODE_LABLE "ModeInfo:"
    ...
    ASSERT(!(val.count & 0x3), "STREAM_CONFIGURATION (%d) should multiple of 4", val.count);
    ...
    {
        LOGI("%s: %08d x %08d  %s", GetFormatStr(val.data.i32[i]), val.data.i32[i + 1], val.data.i32[i + 2], val.data.i32[i + 3] ? "INPUT" : "OUTPUT");
    }
#undef MODE_LABLE
}
    ...
    for (int i = 0; i < val.count; ++i)
    {
        switch (val.type)
        {
            ...
            case ACAMERA_TYPE_RATIONAL:
                LOGI("%s %s: %08x, %08x", printLabel, name, val.data.r[i].numerator, val.data.r[i].denominator);
                break;
            default:
...
void PrintRequestMetadata(ACaptureRequest* req)
{
    ...
    int32_t         count;
    const uint32_t* tags;
    ACaptureRequest_getAllTags(req, &count, &tags);
    for (int32_t idx = 0; idx < count; ++idx)
    {
        ACameraMetadata_const_entry val;
        ACaptureRequest_getConstEntry(req, tags[idx], &val);
        const char* name =
        ...
        for (int i = 0; i < val.count; ++i)
        {
            switch (val.type)
            {
                ...
                case ACAMERA_TYPE_DOUBLE:
                    ...
                    break;
                case ACAMERA_TYPE_RATIONAL:
                    LOGI("Capture Tag %s: %08x, %08x", name, val.data.r[i].numerator, val.data.r[i].denominator);
                    break;
                default:
                    ASSERT(false, "Unknown tag value type: %d", val.type);
            }
