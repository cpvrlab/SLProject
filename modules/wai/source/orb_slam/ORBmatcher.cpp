...
int ORBmatcher::SearchByProjection(WAIFrame& F, const vector<WAIMapPoint*>& vpMapPoints, const float th)
{
    //for every map point
    int nmatches = 0;
    ...
    {
        WAIMapPoint* pMP = vpMapPoints[iMP];
...
int ORBmatcher::SearchByProjection(WAIKeyFrame* pKF, cv::Mat Scw, const vector<WAIMapPoint*>& vpPoints, vector<WAIMapPoint*>& vpMatched, int th)
{
    ...
    // Set of MapPoints already found in the WAIKeyFrame
    set<WAIMapPoint*> spAlreadyFound(vpMatched.begin(), vpMatched.end());
    spAlreadyFound.erase(static_cast<WAIMapPoint*>(NULL));

    int nmatches = 0;

    // For each Candidate WAIMapPoint Project and Match
    for (int iMP = 0, iendMP = (int)vpPoints.size(); iMP < iendMP; iMP++)
    {
        WAIMapPoint* pMP = vpPoints[iMP];
...
int ORBmatcher::SearchForInitialization(WAIFrame& F1, WAIFrame& F2, vector<cv::Point2f>& vbPrevMatched, vector<int>& vnMatches12, int windowSize)
{
    int nmatches = 0;
    ...
    for (size_t i1 = 0, iend1 = F1.mvKeysUn.size(); i1 < iend1; i1++)
    {
        ...
        if (bestDist <= TH_LOW)
        {
            if (bestDist < mfNNratio * (float)bestDist2)
            {
...
int ORBmatcher::SearchForMarkerMap(WAIFrame& F1, WAIKeyFrame& F2, vector<int>& vnMatches12)
{
    int nmatches = 0;
    vnMatches12  = vector<int>(F1.mvKeysUn.size(), -1);
    ...
    for (size_t i1 = 0, iend1 = F1.mvKeysUn.size(); i1 < iend1; i1++)
    {
        ...
        for (size_t i2 = 0; i2 < F2.mDescriptors.rows; i2++)
        {
            cv::Mat d2 = F2.mDescriptors.row((int)i2);
...
int ORBmatcher::SearchByBoW(WAIKeyFrame* pKF1, WAIKeyFrame* pKF2, vector<WAIMapPoint*>& vpMatches12)
{
    ...
    vpMatches12 = vector<WAIMapPoint*>(vpMapPoints1.size(), static_cast<WAIMapPoint*>(NULL));
    ...
    while (f1it != f1end && f2it != f2end)
    {
        if (f1it->first == f2it->first)
        {
            for (size_t i1 = 0, iend1 = f1it->second.size(); i1 < iend1; i1++)
            {
                ...
                WAIMapPoint* pMP1 = vpMapPoints1[idx1];
                if (!pMP1)
                    continue;
                if (pMP1->isBad())
                    continue;
                ...
                for (size_t i2 = 0, iend2 = f2it->second.size(); i2 < iend2; i2++)
                {
                    ...
                    WAIMapPoint* pMP2 = vpMapPoints2[idx2];
    ...
    if (mbCheckOrientation)
    {
        ...
        for (int i = 0; i < HISTO_LENGTH; i++)
        {
            ...
            for (size_t j = 0, jend = rotHist[i].size(); j < jend; j++)
            {
                vpMatches12[rotHist[i][j]] = static_cast<WAIMapPoint*>(NULL);
                nmatches--;
            }
        }
    }
    ...
    while (f1it != f1end && f2it != f2end)
    {
        if (f1it->first == f2it->first)
        {
            for (size_t i1 = 0, iend1 = f1it->second.size(); i1 < iend1; i1++)
            {
                ...
                WAIMapPoint* pMP1 = pKF1->GetMapPoint(idx1);

                // If there is already a WAIMapPoint skip
                if (pMP1)
                    continue;
                ...
                const cv::Mat& d1 = pKF1->mDescriptors.row((int)idx1);
                ...
                for (size_t i2 = 0, iend2 = f2it->second.size(); i2 < iend2; i2++)
                {
                    ...
                    WAIMapPoint* pMP2 = pKF2->GetMapPoint(idx2);
...
int ORBmatcher::Fuse(WAIMap * map, WAIKeyFrame* pKF, const vector<WAIMapPoint*>& vpMapPoints, const float th)
{
    ...
    for (int i = 0; i < nMPs; i++)
    {
        ...
        // If there is already a WAIMapPoint replace otherwise add new measurement
        if (bestDist <= TH_LOW)
        {
            WAIMapPoint* pMPinKF = pKF->GetMapPoint(bestIdx);
            if (pMPinKF)
            {
                if (!pMPinKF->isBad())
                {
...
int ORBmatcher::Fuse(WAIKeyFrame* pKF, cv::Mat Scw, const vector<WAIMapPoint*>& vpPoints, float th, vector<WAIMapPoint*>& vpReplacePoint)
{
    // Get Calibration Parameters for later projection
    const float& fx = pKF->fx;
    const float& fy = pKF->fy;
    ...
    // Set of MapPoints already found in the WAIKeyFrame
    const set<WAIMapPoint*> spAlreadyFound = pKF->GetMapPoints();
...
int ORBmatcher::SearchBySim3(WAIKeyFrame* pKF1, WAIKeyFrame* pKF2, vector<WAIMapPoint*>& vpMatches12, const float& s12, const cv::Mat& R12, const cv::Mat& t12, const float th)
{
    ...
    {
        WAIMapPoint* pMP = vpMapPoints1[i1];
    ...
    {
        WAIMapPoint* pMP = vpMapPoints2[i2];
...
int ORBmatcher::SearchByProjection(WAIFrame& CurrentFrame, const WAIFrame& LastFrame, const float th, const bool bMono)
{
    int nmatches = 0;
    ...
    {
        WAIMapPoint* pMP = LastFrame.mvpMapPoints[i];
    ...
    if (mbCheckOrientation)
    {
        ...
        for (int i = 0; i < HISTO_LENGTH; i++)
        {
            if (i != ind1 && i != ind2 && i != ind3)
            {
                for (size_t j = 0, jend = rotHist[i].size(); j < jend; j++)
                {
                    CurrentFrame.mvpMapPoints[rotHist[i][j]] = static_cast<WAIMapPoint*>(NULL);
                    nmatches--;
                }
            }
        }
...
int ORBmatcher::SearchByProjection(WAIFrame& CurrentFrame, WAIKeyFrame* pKF, const set<WAIMapPoint*>& sAlreadyFound, const float th, const int ORBdist)
{
    int nmatches = 0;
