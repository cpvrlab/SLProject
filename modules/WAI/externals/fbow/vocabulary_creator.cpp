...
void VocabularyCreator::create(fbow::Vocabulary& Voc, const std::vector<cv::Mat>& features, const string& desc_name, Params params)
{
    ...
    _descNBytes      = features[0].cols * (int)features[0].elemSize();
...
void VocabularyCreator::createLevel(int parent, int curL, bool recursive)
{
    ...
    new_nodes.reserve(center_features.size());
    {
        for (size_t c = 0; c < center_features.size(); c++)
            new_nodes.push_back(Node((uint32_t)(parent * _params.k + 1 + c),
                                     (uint32_t)parent,
                                     center_features[c],
                                     findices.size() == center_features.size() ? findices[c] : std::numeric_limits<uint32_t>::max()));
    }
    ...
    if (((_params.L != -1 && curL < (_params.L - 1)) || _params.L == -1) && assigments_ref.size() > 0)
    {
        ...
        if (recursive)
        { //recursive mode(one thread only)
            for (size_t i = 0; i < new_nodes.size(); i++)
                createLevel(parent * _params.k + 1 + (int)i, curL + 1);
        }
        else
        { //parallel mode (multiple theads)
...
#pragma omp parallel for
        ...
        {
            if (_descType == CV_8UC1)
                centers[i] = meanValue_binary(*assigments[i]);
            else
                centers[i] = meanValue_float(*assigments[i]);
        }
    }
    else
    {
        ...
        {
            if (_descType == CV_8UC1)
                centers.push_back(meanValue_binary(*ass));
            else
                centers.push_back(meanValue_float(*ass));
        }
    }
    return centers;
}
...
void VocabularyCreator::convertIntoVoc(Vocabulary& Voc, std::string desc_name)
{
    ...
    for (auto& node : TheTree.getNodes())
    {
        if (node.second.isLeaf())
        {
            ...
            nodeid_blockid.insert(std::make_pair(node.first, nonLeafNodes++));
    }
    ...
    for (auto& node : TheTree.getNodes())
    {
        if (!node.second.isLeaf())
        {
            ...
            for (size_t c = 0; c < node.second.children.size(); c++)
            {
                ...
                binfo.setFeature((int)c, child.feature);
                //go to the end and set info
                if (child.isLeaf())

                    binfo.getBlockNodeInfo((int)c)->setLeaf(child.feat_idx, child.weight);
                else
                {
                    binfo.getBlockNodeInfo((int)c)->setNonLeaf(nodeid_blockid[child.id]);
...
float VocabularyCreator::distance_hamming_32bytes(const cv::Mat& a, const cv::Mat& b)
{
    ...
    return (float)uint64_popcnt(pa[0] ^ pb[0]) + uint64_popcnt(pa[1] ^ pb[1]) + uint64_popcnt(pa[2] ^ pb[2]) + uint64_popcnt(pa[3] ^ pb[3]);
}
