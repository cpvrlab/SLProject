...
std::vector<int> select_closest_features(std::vector<cv::KeyPoint>& keypoints, float radius, int x, int y)
{
    ...
    {
        selection.push_back(sorter[i].idx);
    }
...
std::vector<cv::Mat> image_gradient(const cv::Mat& input_rgb_image)
{
    ...
    {
        cv::sepFilter2D(color_channels[idxC], gx[idxC], CV_32F, d1, p, cv::Point(-1, -1), 0, cv::BORDER_REFLECT);
        cv::sepFilter2D(color_channels[idxC], gy[idxC], CV_32F, p, d1, cv::Point(-1, -1), 0, cv::BORDER_REFLECT);
