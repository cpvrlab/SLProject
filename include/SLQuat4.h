...
{  
 ...
 static SLQuat4<T>  fromLookRotation(const SLVec3<T>& forward, const SLVec3<T>& up);
        ...
        void        toEulerAnglesRPY   (T& pitchRAD, T& yawRAD, T& rollRAD) const;
        void        toEulerAnglesYPR(T& pitchRAD, T& yawRAD, T& rollRAD) const;
...
//-----------------------------------------------------------------------------
template <typename T>
void SLQuat4<T>::toEulerAnglesRPY(T& pitchRAD, T& yawRAD, T& rollRAD) const
{
...
//-----------------------------------------------------------------------------
template <typename T>
void SLQuat4<T>::toEulerAnglesYPR(T& pitchRAD, T& yawRAD, T& rollRAD) const
{
    auto R = this->toMat3();
    R.print("SLProject: \n");

    //// yaw (z-axis rotation)
    //yawRAD = atan2(R[3], R[4]);

    //// pitch (y-axis rotation)
    //pitchRAD = asin(-R[5]);

    //// roll (x-axis rotation)
    //rollRAD = atan2(-R[2], R[8]);
    ...
    // roll (x-axis rotation)
    double sinr = -(T)2 * (_x * _z - _w * _y );
    double cosr = 1 - (T)2 * (_x * _x  + _y * _y );
    rollRAD = atan2( sinr, cosr );
}
//-----------------------------------------------------------------------------
template<class T>
