...
//-----------------------------------------------------------------------------
bool ARCalibration::loadCalibrationParams(std::string calibFilesDir)
{
    //load camera parameter
    FileStorage fs;
    ...
    if (!fs.isOpened())
    {
        cout << "Could not open the calibration parameter file" << endl;
        _state = IDLE;
        return false;
    }
...
//-----------------------------------------------------------------------------
void ARCalibration::calculateCameraFieldOfView()
{
...
static bool runCalibration( Size& imageSize, Mat& cameraMatrix, Mat& distCoeffs,
                            ...
                            vector<float>& reprojErrs, double& totalAvgErr, Size& boardSize, float squareSize, int flag )
{
    //! [fixed_aspect]
    ...
    double rms = calibrateCamera(objectPoints, imagePoints, imageSize, cameraMatrix, distCoeffs, rvecs, tvecs, flag);
...
static void saveCameraParams( Size& imageSize, Mat& cameraMatrix, Mat& distCoeffs,
                              ...
                              double totalAvgErr, int flag, string filename, Size& boardSize, float squareSize )
{
