                 ...
                 inout vec4 Is)     // Specular light intensity
{
    ...
    if(s_shadowsEnabled){
      // Calculate shadow:
      // offset along normal just a teeny tiny bit. This improves shadow quality by removing ugly self-shadowing artefacts:
      ...
      // For that reason, we offset the light away from this voxel. (This of corse offsets the shadows and introduces a small error)
      vec3 to = (u_lightPosVS[0].xyz + normalize(u_lightSpotDirWS[i]) * VOXEL_SIZE * SQRT3) - from;

      // stop sampling for shadows 1 voxel diagonal before reaching the light (again to avoid sampling a lights mesh)
      shadow = coneTraceStopDist(from, to, s_shadowConeAngle, length(to) - VOXEL_SIZE * SQRT3 * s_lightMeshSize , to , 1);
    }
