//#############################################################################
//  File:      PerPixBlinnSm8Cm.frag
//  Purpose:   GLSL pixel shader for per pixel Blinn-Phong lighting with 
//             shadow mapping for max. 8 lights incl. cubemaps for point lights
//             by Joey de Vries.
//  Author:    Michel Schertenleib, Marcus Hudritsch
//  Date:      Winter 2019
...
uniform samplerCube u_shadowMapCube_8;  // cubemap for light 0
uniform samplerCube u_shadowMapCube_9;  // cubemap for light 1
uniform samplerCube u_shadowMapCube_10; // cubemap for light 2
uniform samplerCube u_shadowMapCube_11; // cubemap for light 3
uniform samplerCube u_shadowMapCube_12; // cubemap for light 4
uniform samplerCube u_shadowMapCube_13; // cubemap for light 5
uniform samplerCube u_shadowMapCube_14; // cubemap for light 6
uniform samplerCube u_shadowMapCube_15; // cubemap for light 7
...
float shadowTest(in int i, in vec3 N, in vec3 lightDir)
{
    if (u_lightCreatesShadows[i])
    {
        ...
        else
        {
            if (u_lightUsesCubemap[i])
            {
                if (i == 0) closestDepth = texture(u_shadowMapCube_8, lightToFragment).r;
                if (i == 1) closestDepth = texture(u_shadowMapCube_9, lightToFragment).r;
                if (i == 2) closestDepth = texture(u_shadowMapCube_10, lightToFragment).r;
                if (i == 3) closestDepth = texture(u_shadowMapCube_11, lightToFragment).r;
                if (i == 4) closestDepth = texture(u_shadowMapCube_12, lightToFragment).r;
                if (i == 5) closestDepth = texture(u_shadowMapCube_13, lightToFragment).r;
                if (i == 6) closestDepth = texture(u_shadowMapCube_14, lightToFragment).r;
                if (i == 7) closestDepth = texture(u_shadowMapCube_15, lightToFragment).r;
            }
            else
            {
